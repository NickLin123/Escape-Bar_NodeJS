...
WHERE p_img.`STATUS`='M' AND p.`P_ID`=21 LIMIT 1
...
SELECT p.`PRO_NAME`, COUNT(rating) COMMENTS, AVG(rating) RATING FROM `products` p JOIN `comment` c ON p.`PRO_SEQ`=c.`gid` WHERE gid=1
SELECT p.`PRO_NAME`, c.COUNT(rating) COMMENTS, c.AVG(rating) RATING FROM `products` p JOIN `comment` c ON p.`PRO_SEQ`=c.`gid` WHERE gid=1
SELECT p.`PRO_SEQ`, p.`PRO_NAME`, COUNT(rating) COMMENTS, AVG(rating) RATING FROM `products` p JOIN `comment` c ON p.`PRO_SEQ`=c.`gid` WHERE P_ID=21


SELECT p.`PRO_SEQ`, p.`PRO_NAME`, r.`comments`, r.`rating` FROM `products` p JOIN `totalcomment` r ON p.`PRO_SEQ`=r.`id` WHERE P_ID=21
